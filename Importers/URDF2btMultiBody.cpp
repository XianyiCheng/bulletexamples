...
#include "Importers/ImportURDFDemo/MultiBodyCreationInterface.h"
#include <string>
...
btTransform URDF2BulletMultiBodyInternal(
	...
	int flags, UrdfVisualShapeCache* cachedLinkGraphicsShapesIn, UrdfVisualShapeCache* cachedLinkGraphicsShapesOut, bool recursive)
{
	...
	if (recursive)
	{
		for (int i = 0; i < numChildren; i++)
		{
			...
			URDF2BulletMultiBodyInternal(u2b, creation, cache, urdfChildLinkIndex, linkTransformInWorldSpace, createMultiBody, pathPrefix, flags, cachedLinkGraphicsShapesIn, cachedLinkGraphicsShapesOut, recursive);
		}
	}
	...
	bool createMultiBody, const char* pathPrefix, int flags, UrdfVisualShapeCache* cachedLinkGraphicsShapes)
{
	...
	B3_PROFILE("URDF2BulletMultiBody");
	...
	else
	{
		...
		for (int i = 0; i < allIndices.size(); i++)
		{
			...
			btTransform tr = URDF2BulletMultiBodyInternal(u2b, creation, cache, urdfLinkIndex, parentTr , createMultiBody, pathPrefix, flags, cachedLinkGraphicsShapes, &cachedLinkGraphicsShapesOut, recursive);
