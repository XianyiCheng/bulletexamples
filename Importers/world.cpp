...
void World::importURDF(const char* fileName){


	int flags = 0;
	double globalScaling = 10;

	BulletURDFImporter u2b(this->m_guihelper, 0, 0, globalScaling, flags);
	...
	if (loadOk)
	{
        ...
        btTransform base_tr(btQuaternion(0, 0, 0, 1), btVector3(0,2,0));

		{
			//todo: move these internal API called inside the 'ConvertURDF2Bullet' call, hidden from the user
			//int rootLinkIndex = u2b.getRootLinkIndex();
			//b3Printf("urdf root link index = %d\n",rootLinkIndex);
			MyMultiBodyCreator creation(this->m_guihelper);

			URDF2BulletMultiBody(u2b, creation, base_tr, 1, u2b.getPathPrefix());
			this->mb = creation.getBulletMultiBody();

            // add collision objects to collision world
            this->addCollisionObject(this->mb->getBaseCollider());

            // let all joints to be 0
            // btTransform base_tr(btQuaternion(0, 0, 0, 1), btVector3(0,2,0));
            // this->mb->setBaseWorldTransform(base_tr);
            for (int i = 0; i < this->mb->getNumLinks(); i++){
                this->mb->setJointPos(i, 0);
                this->addCollisionObject(this->mb->getLinkCollider(i));
                // this->mb->setJointPos(i, 0.0);
            }
            
            

            // forward kinematics, update collision objects, update graphics
			btAlignedObjectArray<btQuaternion> scratch_q;
		    btAlignedObjectArray<btVector3> scratch_m;
		    this->mb->forwardKinematics(scratch_q, scratch_m);
		    this->mb->updateCollisionObjectWorldTransforms(scratch_q, scratch_m);
		}
    }
